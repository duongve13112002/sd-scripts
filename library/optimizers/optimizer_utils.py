...
try:
    ...
    QBytesTensor = None  # optimum-quanto not installed, quantized tensor support disabled
...
def update_parameter(target, result_float):
    ...
    """
    if QBytesTensor is not None and isinstance(target, QBytesTensor):
...
def copy_stochastic(target: torch.Tensor, source: torch.Tensor, eps: Optional[float] = None) -> None:
    """
    Copy with stochastic rounding.

    FIXED: Now handles CPU gracefully instead of crashing.
    """
    with torch.no_grad():
        # ==================== FIX: Handle CPU ====================
        # Stochastic rounding only beneficial on GPU
        if target.device.type == 'cpu' or source.device.type == 'cpu':
