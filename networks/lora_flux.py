...
class DoRAModule(LoRAModule):
    ...
    def __init__(
        ...
        # Weight shape: [out_features, in_features] for Linear
    ...
    def apply_to(self):
        ...
        # Using original norms (not combined) matches ComfyUI's weight_decompose convention
    ...
    def forward(self, x):
        ...
        if self.fp32_accumulation:
            lx = torch.nn.functional.linear(lx, self.lora_up.weight.float())
            # Cast LoRA delta back to native dtype early — fp32 only needed for the small rank matmuls
        ...
        # This matches ComfyUI's weight_decompose: result = (dora_scale / ||W||) * (W + ΔW) @ x
        # _org_weight_norm is frozen (computed once in apply_to), so gradient only flows through magnitude
        mag_scale = (self.magnitude / self._org_weight_norm).to(org_out.dtype)

        # Broadcast mag_scale to match output shape: [out_dim] -> appropriate dims
