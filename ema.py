...
import copy
import contextlib
from library.optimizers.optimizer_utils import copy_stochastic
...
class ExponentialMovingAverage:
    ...
    def __init__(
            ...
            accelerator=None,  # ← NEW: Required for multi-GPU
    ):
    ...
    #     Called before sampling/evaluation.
    #     """
    #     if self.accelerator is None or self.num_processes == 1:
    #         return  # Single GPU, no sync needed
        
    #     if not self.is_main_process and self.shadow_params is None:
    #         # Workers: Create empty shadow params to receive broadcast
    #         self.shadow_params = [
    #             torch.zeros(shape, dtype=dtype, device=self.device)
    #             for shape, dtype in zip(self.shadow_param_shapes, self.shadow_param_dtypes)
    #         ]
        
    #     # Broadcast each shadow param from main (rank 0) to all workers
    #     for i, shadow_param in enumerate(self.shadow_params):
    #         # Move to GPU temporarily for efficient broadcast
    #         device_for_broadcast = self.accelerator.device
    #         shadow_on_gpu = shadow_param.to(device_for_broadcast)
    ...
    #         self.accelerator.print("✓ EMA: Worker received shadow parameters from main process")
